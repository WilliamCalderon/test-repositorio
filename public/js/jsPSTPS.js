...
$(document).ready(function() {
    ...
    tinymce.init({
        selector: '#tbx_situacion_deseada',
        menubar: false,
        plugins: [
            'advlist autolink lists link image charmap print preview anchor',
            'searchreplace visualblocks code fullscreen',
...
$('#btn-nuevo-objetivo').bind('click', async e => {
    ...
    var html = await $.ajax({
        ...
        url: site +"/pstps/ModalObjetivo/Agregar/0"
    });
...
async function EditarObjetivo(id_objetivo) {
    ...
    var html = await $.ajax({
        ...
        url: site +"/pstps/ModalObjetivo/Modificar/"+id_objetivo
    });
...
$('#btn-nueva-estrategia').bind('click', async e => {
    ...
    var html = await $.ajax({
        ...
        url: site +"/pstps/ModalEstrategia/Agregar/0"
    });
...
async function EditarEstrategia(id_estrategia) {
    ...
    var html = await $.ajax({
        ...
        url: site + "/pstps/ModalEstrategia/Modificar/"+id_estrategia
    });
...
$('#btn-nueva-accion').bind('click', async e => {
    ...
    var html = await $.ajax({
        ...
        url: site +"/pstps/ModalAccion/Agregar/0"
    });
...
async function EditarAccion(id_accion) {
    ...
    var html = await $.ajax({
        ...
        url: site + "/pstps/ModalAccion/Modificar/"+id_accion
    });
...
function nuevoDocumento(id_seccion_docto, nombre_seccion) {
    $.ajax({
            ...
            url: site +"/pstps/CargarCategriasDoctos/" + id_seccion_docto,
            success: function(respuesta) {
...
$('#btn-nuevo-indicador').bind('click', async e => {
    ...
    var html = await $.ajax({
        ...
        url: site +"/indicador/ModalAgregarIndicador/2"
    });
    ajaxcontent.innerHTML += html;
...
async function editarDatosGeneralesIndicador(id_indicador) {
    ...
    var html = await $.ajax({
        ...
        url: site + "/indicador/ModalEditarIndicador/2"
    });
...
async function editarElementosMetaIndicador(id_indicador) {
    ...
    var html = await $.ajax({
        ...
        url: site + "/indicador/ModalElementosMeta/2"
    });
...
async function editarLineaBaseIndicador(id_indicador) {
    ...
    var html = await $.ajax({
        ...
        url: site +"/indicador/ModalLineaBase/2"
    });
...
async function editarSerieHistoricaIndicador(id_indicador) {
    ...
    var html = await $.ajax({
        ...
        url: site + "/indicador/ModalSerieHistorica/2"
    });
...
async function editarMetasIntermediasIndicador(id_indicador) {
    ...
    var html = await $.ajax({
        ...
        url: site + "/indicador/ModalMetasIntermedias/2"
    });
...
async function editarAplicacionDelMetodo(id_indicador) {
    ...
    var html = await $.ajax({
        ...
        url: site + "/indicador/ModalVariables/2"
    });
    ajaxcontent.innerHTML += html;
    $('#cuerpo-content').append(ajaxcontent);
...
function AgregarActualizarVariable(){
    ...
    $.ajax({
		...
		url: site + "/variable/AgregarActualizarVariable/pstps",
		data: ArrayPOST,
		success:function(respuesta){
            $("body").dynamicSpinnerDestroy({});      
            if(respuesta==1){
...
function ActualizarPrograma(){
    var ArrayPOST = { 
        ...
        "tbx_situacion_deseada"         : tinymce.get("tbx_situacion_deseada").getContent(),
        "tbx_fundamento_normativo"      : ""
    };
    ...
    $.ajax({
		...
		url: site + "/pstps/ActualizarPrograma",
		data: ArrayPOST,
		success:function(respuesta){
            $("body").dynamicSpinnerDestroy({});      
            if(respuesta==1){
    ...
    $.ajax({
        ...
        success: function(respuesta) {
            ...
            if(datos.situacion_deseada != null)
                tinymce.get('tbx_situacion_deseada').setContent(datos.situacion_deseada);
            ...
            if(datos.length > 0){
                datos.forEach(element => {
                    var celdas ='<tr><td>'+element.row+'</td>'+
                                     ...
                                     '<td align="center">'+
                                            '<a href="'+site+'/public/documentos/pstps/'+element.documento_foro+'" target="_blank" data-toggle="tooltip" data-placement="top" title="Visualizar documento"><span class="material-icons">preview</span></a>'+
...
$('#btn-nuevo-ducumento-pd').bind('click', async e => {
    ...
    var html = await $.ajax({
        ...
        url: site +"/pstps/ModalParticipacionDemocratica/0"
    });
